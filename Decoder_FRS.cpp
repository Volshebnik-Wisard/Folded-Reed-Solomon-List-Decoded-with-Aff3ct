...
::Decoder_FRS(const int& K, const int& N, const tools::RS_polynomial_generator& GF)
	...
	err(this->N_p2_1)
{
	const std::string name = "Decoder_FRS";
	this->set_name(name);
}

template <typename B, typename R>
Decoder_FRS<B, R>* Decoder_FRS<B, R>
::clone() const
{
	auto m = new Decoder_FRS(*this);
	m->deep_copy(*this);
	return m;
}
...
template class aff3ct::module::Decoder_FRS<B_8, Q_8>;
template class aff3ct::module::Decoder_FRS<B_16, Q_16>;
template class aff3ct::module::Decoder_FRS<B_32, Q_32>;
template class aff3ct::module::Decoder_FRS<B_64, Q_64>;
#else
template class aff3ct::module::Decoder_FRS<B, Q>;
#endif
// ==================================================================================== explicit template instantiation
